...
#include <xbee/cmd_ctrl.h>
 ...
 TeleopMQS::TeleopMQS():
  ...
  wrt(3), //wheel retraction binary on Y
  cp(4), //cooling pump binary on LB
  ...
  abort(8) //abort to RC transmitter on Xbox button
 {
   ...
   nh_.param("shift_steer_marine",strm_shift,strm_scale);
   nh_.param("shift_reverse",rev_shift,rev_scale);
   nh_.param("shift_steer_land",strl_shift,strl_scale);
   nh_.param("shift_throttle",thl_shift,thl_scale);
   nh_.param("shift_forward",fwd_shift,fwd_scale);
