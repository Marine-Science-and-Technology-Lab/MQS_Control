  ...
  parent_frame (p_frame)
{
  ...
  pub_filter = nh_ptr->advertise<nav_msgs::Odometry>(subject_name+"/odom", 1);
  pub_raw = nh_ptr->advertise<geometry_msgs::PoseStamped>(subject_name+"/pose", 1);
  pub_vel = nh_ptr->advertise<geometry_msgs::TwistStamped>(subject_name+"/velocity", 1);
  return;
}

Marker::Marker(ros::NodeHandle* nmptr, const string& mark_name):
  marker_name        (mark_name),
  marker_status      (LOST),
  mnh_ptr      (nmptr)
{
  pub_raw = mnh_ptr->advertise<geometry_msgs::PoseStamped>(marker_name+"/point", 1);
  return;
}
...
// Get and set name of the marker
const string& Marker::getName() {
  boost::shared_lock<boost::shared_mutex> read_lock(mtx);
  return marker_name;
}
void Marker::setName(const string& mark_name) {
  boost::unique_lock<boost::shared_mutex> write_lock(mtx);
  marker_name = mark_name;
}

// Enable or diable the marker
const Marker::Status& Marker::getStatus() {
  boost::shared_lock<boost::shared_mutex> read_lock(mtx);
  return marker_status;
}
void Marker::enable() {
  boost::unique_lock<boost::shared_mutex> write_lock(mtx);
  marker_status = INITIALIZING;
  ROS_INFO("Initializing subject %s", marker_name.c_str());
}
void Marker::disable() {
  boost::unique_lock<boost::shared_mutex> write_lock(mtx);
  kFilter.reset();
  marker_status = LOST;
  ROS_WARN("Lost track of subject %s", marker_name.c_str());
}
...
const Vector3d& Marker::getPosition() {
  boost::shared_lock<boost::shared_mutex> read_lock(mtx);
  return kFilter.position;
}
...
void Subject::processNewMeasurement(
  ...
  odom_filter.child_frame_id = subject_name + "/base_link";
...
// Process the new marker measurement
void Marker::processNewMeasurement(
    const double& time,
    const Quaterniond& m_attitude,
    const Vector3d& m_position) {
  //ros::Time tbefore_filter = ros::Time::now();
