...
/*
 * @brief Marker Defines attributes of a 3D Marker
 */
class Marker{
  public:
     EIGEN_MAKE_ALIGNED_OPERATOR_NEW
     ...
     * @brief Constructor and Destructor
     */
    Marker(ros::NodeHandle* nptr, const std::string& mark_name);
    ~Marker() {}
     ...
     */
    const std::string& getName();
    void setName(const std::string& mark_name);
    /*
     * @brief isActive Tells if the marker is still active or not
     */
    const Status& getStatus();
    void enable();
    void disable();
    ...
    typedef const boost::shared_ptr<Marker> MarkerConstPtr;
  ...
  private:
    ...
    // Publisher for the marker probably unnecessary 
    ros::NodeHandle* mnh_ptr;
    //std::string parent_frame;
    //ros::Publisher pub_filter;
    ros::Publisher pub_raw;
    //ros::Publisher pub_vel;
...
class MoCapDriverBase{
  ...
  public:
    ...
    MoCapDriverBase(const ros::NodeHandle& n):
      ...
      marker_list    (std::vector<std::string>(0)),
      publish_tf     (false),
      publish_pos    (false),
  ...
  protected:
    ...
    virtual void handleMarker(int mark_idx)=0;
    ...
    // Empty string of all the objects in the arena are to be tracked
    std::vector<std::string> model_list;

    // Marker to be tracked
    //Empty string of all the markers in the arena are to be tracked
    ...
    //Observed markers (contains those lose tracking)
    std::map<std::string, Marker::MarkerPtr> markers;
    ...
    // Publish pos
    bool publish_pos;
    ros::Publisher pos_publisher=nh.advertise<geometry_msgs::Vector3Stamped>("/marker_xyz",1);
