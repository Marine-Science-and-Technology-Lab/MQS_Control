...
bool QualisysDriver::init() {
  ...
  nh.param("marker_list",marker_list,vector<string>(0));
  ...
  nh.param("publish_pos", publish_pos,false);
  ...
  // Get 3D settings
  bool cDataAvailable = false;
  bool xyz_settings = false;
  xyz_settings=port_protocol.Read3DSettings(cDataAvailable);
  if (bDataAvailable == false || xyz_settings==false)
  {
    ROS_FATAL_STREAM("Reading 3D data Settings failed during initialization\n"
                <<"QTM error:"<< port_protocol.GetErrorString());
    return false;
  }
  ...
  cDataAvailable=port_protocol.StreamFrames(
      stream_rate_mode,
      frame_rate, //nRateArg
      udp_stream_port, //nUDPPort
      nullptr, //nUDPAddr
      CRTProtocol::cComponent3d);
  if(!CRTProtocol::cComponent3d)
  {
    ROS_INFO_STREAM("No 3d component data available; returned: " << CRTProtocol::cComponent3d);
    return false;
  }
  else
  {
    ROS_INFO_STREAM("3d component data available; returned: " << CRTProtocol::cComponent3d);
    ROS_INFO("Frame rate: %i frames per second", frame_rate);
    // Calculate covariance matrices
    process_noise.topLeftCorner<6, 6>() =
    0.5*Matrix<double, 6, 6>::Identity()*dt*dt*max_accel;
    ...
    Matrix<double, 6, 6>::Identity()*1e-3;
    measurement_noise *= measurement_noise; // Make it a covariance  
    marker_set.insert(marker_list.begin(),marker_list.end());
  }
  if(bDataAvailable==true && cDataAvailable==true)
  {
    return true;
  }
  else{
    ROS_INFO_STREAM("One or more component data type unavailable");
    return false;
  }
}
...
void QualisysDriver::handleFrame() {
  ...
  //number of markers
  int marker_count=prt_packet->Get3DMarkerCount();
  //int component_count=prt_packet->GetComponentCount();
  //int component_size=prt_packet->GetComponentSize(CRTPacket::Component3d);
  //ROS_INFO_STREAM("Number of 6DOF bodies: "<<body_count<<" Number of 3D Markers: "<<marker_count<<" Components size of type 3D: "<<component_size);
  ...
  {
   string marker_name(port_protocol.Get3DLabelName(i));
   //ROS_INFO_STREAM("Marker name: "<<marker_name);
   // process if required
   if(marker_set.empty() || marker_set.count(marker_name))
   {
     if(markers.find(marker_name)==markers.end())
     {
       markers[marker_name]=Marker::MarkerPtr(new Marker(&nh, marker_name));
       markers[marker_name]->setParameters(process_noise, measurement_noise, frame_rate);
     }
     handleMarker(i);	
   }
  }
...
void QualisysDriver::handleMarker(int mark_idx) {
  // Name of the subject marker
  string marker_name(port_protocol.Get3DLabelName(mark_idx));
  // Pos of subject marker
  //ROS_INFO_STREAM("marker name: "<<component_name);
  const unsigned int matrix_size = 3;
  float x, y, z;
  float rot_array[matrix_size*matrix_size]={1,0,0,0,1,0,0,0,1}; //static rotation matrix
  prt_packet->Get3DMarker(mark_idx, x, y, z);

  // Check if the marker is tracked by looking for NaN in the received data
  if(isnan(x) || isnan(y) || isnan(z)) {
    if(markers.at(marker_name)->getStatus() != Marker::LOST){
      markers.at(marker_name)->disable();
    }
    return;
  }
  ...
  // Convert the rotation matrix to a quaternion
  Eigen::Matrix<float, 3, 3, Eigen::ColMajor> rot_matrix(rot_array);
  Eigen::Quaterniond mark_att(rot_matrix.cast<double>());
  // Convert mm to m
  Eigen::Vector3d mark_pos(x/1000.0, y/1000.0, z/1000.0);
  //ROS_INFO_STREAM("Position x,y,z: "<<pos_vector[0]<<","<<pos_vector[1]<<","<<pos_vector[2]);

  // Re-enable the object if it is lost previously
  if (markers.at(marker_name)->getStatus() == Subject::LOST) {
    markers.at(marker_name)->enable();
  }
  ...
  // Feed the new measurement to the subject
  markers.at(marker_name)->processNewMeasurement(time,mark_att ,mark_pos);

  // Publish pos if requred
  if (publish_pos &&
      markers.at(marker_name)->getStatus() == Marker::TRACKED) 
  {
    Vector3d pos = markers.at(marker_name)->getPosition();
    ROS_DEBUG("Name: %s,\tindex: %i", marker_name.c_str(), mark_idx);
    ROS_DEBUG("Position\nx: %f,\ty: %f\tz: %f", x/1000.0, y/1000.0, z/1000.0);
    geometry_msgs::Vector3Stamped stamped_vector3;
    tf::vectorEigenToMsg(pos, stamped_vector3.vector);
    stamped_vector3.header.stamp=ros::Time(time);  
    stamped_vector3.header.frame_id=marker_name; 
    pos_publisher.publish(stamped_vector3);
